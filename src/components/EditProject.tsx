...
import { SelectOptionType, Task } from '../types.ts'
...
export function EditProject({ name, managerRoles, tasks, projectId }: Props) {
	...
	const [defaultRoles, setDefaultRoles] = useState<APIRole[]>([])
	...
	useEffect(() => {
		fetch(`/api/roles/${discordSdk.guildId}`)
			...
			.then(roles => {
				...
				setDefaultRoles((roles as APIRole[]).filter(role => managerRoles.includes(role.id)).sort((a, b) => b.position - a.position))
			})
		...
		<div style={{ height: '400px' }}>
			...
			>
				...
				<Select
					...
					value={defaultRoles.map(r => ({ value: r.id, label: r.name, color: transformColor(r.color)}))}
					name="roles"
				/>
